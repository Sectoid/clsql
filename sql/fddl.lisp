...
an error is signalled if IF-DOES-NOT-EXIST is :error."
    ...
    (let ((expr (concats "DROP TABLE " (escaped-database-identifier name database))))
...
an error is signalled if IF-DOES-NOT-EXIST is :error."
    (ecase if-does-not-exist
      ...
      (:error
       t))
    (let ((expr (concats "DROP VIEW " (escaped-database-identifier name database))))
      (execute-command expr :database database)))
